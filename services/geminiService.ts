...
export const generateQuestionsAndAnswers = async (text: string, pdfBase64?: string): Promise<{ question: string; answer: string; }[]> => {
...
Your task is to analyze the provided PDF document content and generate a list of questions and their corresponding answers.

RULES:
- Generate as many relevant Q&A pairs as the content supports.
- Analyze all content in the PDF including text, diagrams, schematics, flowcharts, tables, and images.
- When analyzing visual elements (diagrams, flowcharts, tables):
    - Extract text from images (e.g., labels, values, titles).
    ...
    - Integrate visual understanding with accompanying text to form comprehensive Q&A pairs.
- When generating answers for information found in a table, identify the key attributes (column headers) that describe each row's entry. The answer must concisely summarize ALL relevant information found in the same row, including the values from these identified key attribute columns and the description.
- Questions MUST be specific enough to allow direct retrieval from the provided content. For information originating from a specific section or visual element, the question should implicitly or explicitly reference the relevant part (e.g., "According to the timing diagram, what is the speed of the conveyor?").
- While concise, questions should contain sufficient detail to differentiate between similar concepts if they appear in different contexts within the document.
...
- For text content, answers must be extracted directly from the provided content and be concise.
- If the content is unsuitable for creating Q&A pairs (e.g., it's just a list of names, irrelevant content, or blank pages), return an empty array.
...
${text ? `EXTRACTED TEXT FOR CONTEXT:\n---\n${text}\n---\n` : '(Text extraction may be limited, analyze PDF directly)'}
`;
